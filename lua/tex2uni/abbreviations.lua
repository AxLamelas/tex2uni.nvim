---@mod tex2uni.abbreviations (Unicode) Abbreviation Expansion
...
function abbreviations.show_reverse_lookup()
	...
	if vim.tbl_isempty(results) then
		lines = {
			...
			"  require'tex2uni'.setup{ abbreviations = { extra = { ... } } }`",
		}
	else
		lines = {}
		for i = #char, 1, -1 do
	...
	vim.lsp.util.open_floating_preview(
		lines,
		"markdown",
		{ focus_id = "tex2uni_abbreviation_help", border = "rounded" }
	)
end

local abbr_mark_ns = vim.api.nvim_create_namespace("tex2uni.abbreviations")
...
local function insert_char_pre()
	...
	if not abbreviations.abbr_mark and char == abbreviations.leader then
		...
		abbreviations.abbr_mark = vim.api.nvim_buf_set_extmark(0, abbr_mark_ns, row, col, {
			hl_group = "tex2uniAbbreviationMark",
			end_line = row,
			end_col = col,
			right_gravity = false,
		...
		local mappings = {
			["<CR>"] = [[<C-o>:lua require'tex2uni.abbreviations'.convert()<CR><CR>]],
			["<Tab>"] = [[<Cmd>lua require'tex2uni.abbreviations'.convert()<CR>]],
		}
...
function abbreviations.enable(pattern, opts)
	...
	vim.cmd([[hi def tex2uniAbbreviationMark cterm=underline gui=underline guisp=Gray]])
end
